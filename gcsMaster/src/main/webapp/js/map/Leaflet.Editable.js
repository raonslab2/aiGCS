...
(function(factory, window) {
			...
			moveMarker.on('drag', function(e) {
				...
				polygon.setLatLngs(latlngs.map(function(part) {
					return part.map(function(point) {
						return [point.lat + translation[0], point.lng + translation[1]];
					});
				}));
				polygon.redraw();
				center = newCenter;
	...
	L.Editable.VertexMarker = L.Marker.extend({
		...
		onAdd: function(map) {
			...
			this.on('mouseover', this.onMouseOver);  // 마우스 오버 이벤트 추가
			this.on('mouseout', this.onMouseOut);    // 마우스 아웃 이벤트 추가
		...
		onRemove: function(map) {
			...
			this.off('mouseover', this.onMouseOver);  // 마우스 오버 이벤트 제거
			this.off('mouseout', this.onMouseOut);    // 마우스 아웃 이벤트 제거
		...
		onMouseOver: function(e) {
			...
			L.DomUtil.addClass(this._icon, 'custom-vertex-icon-hover');  // 마우스 오버 시 클래스 추가
		},

		onMouseOut: function(e) {
			e.vertex = this;
			this.editor.onVertexMarkerMouseOut(e);
			L.DomUtil.removeClass(this._icon, 'custom-vertex-icon-hover');  // 마우스 아웃 시 클래스 제거
		},
	...
	/*  
	  // 새로운 이미지 아이콘 생성
	  var customVertexIcon = new L.Icon({
		  iconUrl: 'your-image-url.png',  // 여기에 이미지의 URL을 입력하세요.
		  iconSize: [24, 24],             // 아이콘 크기 조정
		  iconAnchor: [12, 12]            // 아이콘의 앵커 포인트 조정
	  });
  	
	  // VertexMarker 클래스를 수정하여 사용자 정의 아이콘 사용
  ...
  */
	...
	L.Editable.MiddleMarker = L.Marker.extend({
		...
		options: {
			opacity: 1.0,
			className: 'custom-vertex-icon leaflet-div-icon leaflet-middle-icon',
			draggable: true
		},
