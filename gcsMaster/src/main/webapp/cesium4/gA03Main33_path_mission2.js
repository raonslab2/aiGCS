...
function init_path_mis_mission(mission_allow_edit, mission_has_name) {
    ...
    function path_HomeLonLat(lon, lat) {
        ...
        //$("input.mission-lat").val(Number(lat).toFixed(8)) 
    	//$("input.mission-lon").val(Number(lon).toFixed(8))
        var inpLat = document.getElementsByName("waypointlat[]");
        inpLat[0].value = Number(lat).toFixed(8);
 
        var inpLon = document.getElementsByName("waypointlon[]");
        inpLon[0].value = Number(lon).toFixed(8);
        
        console.log(path_HomeLonLat+"::"+Number(lat).toFixed(8));
        console.log(Number(lon).toFixed(8));
        /*
            $.ajax({
                type: "get",
                url: "http://apis.vworld.kr/coord2jibun.do",
                data : {
                	apiKey : '1690BC36-98F4-3B67-80A3-C8C8CFD3A565',
                	x : Number(lon).toFixed(8),
                ...
                success: function(data) { 
                    ...
                    $("input.mission-addr").val(data.ADDR)
                },
                error: function(xhr, stat, err) {}
            }); 
            
        $("span.mission-lat").text(Number(lat).toFixed(8) + ", ")
        $("span.mission-lon").text(Number(lon).toFixed(8))
*/
    }
