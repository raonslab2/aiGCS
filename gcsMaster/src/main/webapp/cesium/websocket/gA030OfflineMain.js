...
// 온라인 타일 레이어
var onlineTiles = L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
    maxZoom: 19,
		...
		if (list.DATA_GUBUN == 'DRONE_STATE') {
			...
			for (var si = 0; si < list.DATA_STATE.length; si++) {
				...
				if (list.DATA_STATE[si].dl_name == 'lm_10001') {
						$('#drone101x').attr("value", list.DATA_STATE[si].st_x);	
						$('#drone101y').attr("value", list.DATA_STATE[si].st_y);
						$('#drone101z').attr("value", list.DATA_STATE[si].st_z)	;
						$('#drone101r').attr("value", list.DATA_STATE[si].st_roll)	;
						$('#drone101p').attr("value", list.DATA_STATE[si].st_pitch)	;
						$('#drone101y').attr("value", list.DATA_STATE[si].st_yaw)	;
						$('#drone101h').attr("value", list.DATA_STATE[si].st_head)	; 
					}else if (list.DATA_STATE[si].dl_name == 'lm_10002') {
						$('#drone102x').attr("value", list.DATA_STATE[si].st_x);	
						$('#drone102y').attr("value", list.DATA_STATE[si].st_y);
						$('#drone102z').attr("value", list.DATA_STATE[si].st_z)	;
						$('#drone102r').attr("value", list.DATA_STATE[si].st_roll)	;
						$('#drone102p').attr("value", list.DATA_STATE[si].st_pitch)	;
						$('#drone102y').attr("value", list.DATA_STATE[si].st_yaw)	;
						$('#drone102h').attr("value", list.DATA_STATE[si].st_head)	; 
					}else if (list.DATA_STATE[si].dl_name == 'lm_10003') {
						$('#drone103x').attr("value", list.DATA_STATE[si].st_x);	
						$('#drone103y').attr("value", list.DATA_STATE[si].st_y);
						$('#drone103z').attr("value", list.DATA_STATE[si].st_z)	;
						$('#drone103r').attr("value", list.DATA_STATE[si].st_roll)	;
						$('#drone103p').attr("value", list.DATA_STATE[si].st_pitch)	;
						$('#drone103y').attr("value", list.DATA_STATE[si].st_yaw)	;
						$('#drone103h').attr("value", list.DATA_STATE[si].st_head)	; 
					}else if (list.DATA_STATE[si].dl_name == 'lm_10004') {
						$('#drone104x').attr("value", list.DATA_STATE[si].st_x);	
						$('#drone104y').attr("value", list.DATA_STATE[si].st_y);
						$('#drone104z').attr("value", list.DATA_STATE[si].st_z)	;
						$('#drone104r').attr("value", list.DATA_STATE[si].st_roll)	;
						$('#drone104p').attr("value", list.DATA_STATE[si].st_pitch)	;
						$('#drone104y').attr("value", list.DATA_STATE[si].st_yaw)	;
						$('#drone104h').attr("value", list.DATA_STATE[si].st_head)	; 
					}else if (list.DATA_STATE[si].dl_name == 'lm_10005') {
						$('#drone105x').attr("value", list.DATA_STATE[si].st_x);	
						$('#drone105y').attr("value", list.DATA_STATE[si].st_y);
						$('#drone105z').attr("value", list.DATA_STATE[si].st_z)	;
						$('#drone105r').attr("value", list.DATA_STATE[si].st_roll)	;
						$('#drone105p').attr("value", list.DATA_STATE[si].st_pitch)	;
						$('#drone105y').attr("value", list.DATA_STATE[si].st_yaw)	;
						$('#drone105h').attr("value", list.DATA_STATE[si].st_head)	; 
					}
...
function fn_droneAction(obj, str) {
	...
	*/ 
	...
	if(idx_chk.is(':checked')==false){
	    ...
	    // AI SWARM 학습 결과정보
        let drone_total_cnt = chk_arr.length;
        var droneTitle = "";
		$.ajax({
			url : '/gcs/dashboard/getDroneAction.do'
			, type : 'post' 
			, data : { "droneCnt" : drone_total_cnt , "droneAction" : str }
			, async : false
		}).done(function(res) { 
			console.log(res.list);  
			if(drone_total_cnt==1){
				droneMission = "";
			}else{
				if(
					typeof res.list[0] == "undefined"
					|| res.list[0] == null 
					|| res.list[0] == ""
				){
					droneTitle = "null"; 
				}else{
					droneTitle = res.list[0].droneTitle; 
				}
        ...
        console.log("droneTitle::::"+droneTitle);   
        ...
        */
		...
		for(var i=0; i<drone_total_cnt; i++){
			dlName = chk_arr[i];
...
function fn_swarm(dronecnt,i,x,y,z,s){
   

	
	var tmOption = "";
	tmOption = (parseFloat(x)+0.0005*i) + 
	"|" + (parseFloat(y)+0.0005*i) + 
	"|" + (Number(z)+i+3) + 
	"|" + s;
