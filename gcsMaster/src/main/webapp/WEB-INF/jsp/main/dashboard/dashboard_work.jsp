...
<%
	String dataDtlAncmNm = request.getParameter("dtlAncmNm");
	String dataBisiYy = request.getParameter("bisiYy");
%>
...
<form id="searchForm1">
	...
	<input type="hidden" id="searchYear" name="searchYear" value="<%=dataBisiYy %>" />
	<input type="hidden" id="searchName" name="searchName" value="<%=dataDtlAncmNm %>" />
</form>
<!-- 업무요청 -->
			...
			<div class="bbs_list">
				<table id="bbs_list1">
						...
						<tr>
						  ...
						  <th scope="col">세부 공고명</th>
						  <th scope="col">주관기관명</th>
						  <th scope="col">협약체결여부</th>
						  <th scope="col">협약체결일자</th>
						</tr>
						</thead>
...
<script>
;(function() {
	$(document).on('click', '.pageClass1', function() {
		...
		$.ajax({
			url : '/empo/dashboard/popupDetail.do'
			, type : 'post'
			, data :formSer
		}).done(function(res) { 
			console.log(res);
			$('#resultList1').children().remove();
			$('.pagination1').children().remove();
			//목록 출력
			...
			$('.pagination1').append(paginationView(res.paginationInfo));
			
			$('.pagination1').children('.' + res.paginationInfo.currentPageNo).addClass("active");
			//$('.' + res.paginationInfo.currentPageNo ).addClass("active");
			$('.pagination1 > li').css('cursor', 'pointer');
	...
	var paginationView = function(page) {
		 var data = "" ;
		 	...
		 	data = 
			'	<li class="first"><a class="pageClass1" data-page="' + page.firstPageNo + '" title="첫 페이지 바로가기"><img src="/images/bbs_prev02.gif" alt="첫 페이지" style="margin: 10px;"></a></li>' +
			'	<li class="prev"><a class="pageClass1" data-page="' + (page.currentPageNo-1) + '" title="이전 페이지(1page) 바로가기"><img src="/images/bbs_prev01.gif" alt="이전 페이지" style="margin: 10px;"></a></li>' ;
			
			for (var i=page.firstPageNoOnPageList; i <= page.lastPageNoOnPageList; i++) {
				data = data +
					'<li class=' + i + '><a class="pageClass1" data-page="' + i + '" title="' + i + '페이지 바로가기">' + i + '</a></li>' ;
			}
			
			data = data +
				'<li class="next"><a class="pageClass1" data-total_page_count="' + page.totalPageCount + '" data-page="' + (page.currentPageNo+1) + '" title="다음 페이지(11page) 바로가기"><img src="/images/bbs_next01.gif" alt="다음 페이지" style="margin: 10px;"></a></li>' +
				'<li class="last"><a class="pageClass1" data-page="' + page.lastPageNo + '" title="끝 페이지(67page) 바로가기"><img src="/images/bbs_next02.gif" alt="마지막 페이지" style="margin: 10px;"></a></li>' ;
	...
	var projectListView = function(row) {
		...
		var data = 
			...
			'	<td class="subRow">' + row.agreCntrYn + '</td>' +
			'	<td class="subRow">' + row.agreCntrYmd + '</td>' +
			'</tr>' ;
	...
	$(function() {
		...
		$.post("/empo/dashboard/popupDetail.do", formSer).done(function(res) {
