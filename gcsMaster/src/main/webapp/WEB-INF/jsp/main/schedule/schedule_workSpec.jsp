			...
			<div class="bbs_list">
				<table id="bbs_list1">
						...
						<tr>
						  ...
						  <th scope="col">문제과제지정일자</th>
						  <th scope="col">문제과제사유</th>
						  <th scope="col">특별평가일자</th>
						  <th scope="col">위원회명</th>
						  <th scope="col">특별평가점수</th>
						  <th scope="col">특별평가장소</th>
						  <th scope="col">특별평가결과</th>
						</tr>
						</thead>
...
;(function() {
	...
	$(document).on('click', '.pageClass', function() {
		...
		$.ajax({
			url : '/empo/schedule/specProjectList.do'
			, type : 'post'
			, data :formSer
		}).done(function(res) { 
			console.log(res);
	...
	var projectListView = function(row) {
		...
		var data = 
			...
			'	<td class="subRow"></td>' +
			'	<td class="subRow">' + row.prblSbjtRcptYmd  + '</td>' +
			'	<td class="subRow">' + row.prblSbjtRsn + '</td>' +
			'	<td class="subRow">' + row.spevlYmd  + '</td>' +
			'	<td class="subRow">' + row.ecmitNm + '</td>' +
			'	<td class="subRow">' + row.evalGd + '</td>' +
			'	<td class="subRow">' + row.plcNm + '</td>' +
			'	<td class="subRow">' + row.spevlRsltNm + '</td>' +
			'</tr>' ;
	...
	$(function() {
		...
		$.post('/empo/schedule/specProjectList.do', formSer).done(function(res) {
