			...
			<div class="bbs_list">
				<table id="bbs_list1">
						...
						<tr>
						  ...
						  <th scope="col">최종보고서접수마감일자</th>
						  <th scope="col">최종보고서제출일자</th>
...
;(function() {
	$(document).on('click', '.pageClass', function() {
		...
		$.ajax({
			url : '/empo/schedule/reportProjectList.do'
			, type : 'post'
			, data :formSer
		}).done(function(res) { 
			console.log(res);
	...
	var projectListView = function(row) {
		...
		var data = 
			...
			'	<td class="subRow"></td>' +
			'	<td class="subRow"></td>' +
			'	<td class="subRow">' + row.lastRptpSubmYmd  + '</td>' +
	...
	$(function() { 
		var formSer = $("#searchForm1").serialize();
		//과제리스트 호출
		$.post('/empo/schedule/reportProjectList.do', formSer).done(function(res) {
