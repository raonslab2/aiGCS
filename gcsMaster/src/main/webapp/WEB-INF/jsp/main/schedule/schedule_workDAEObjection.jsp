			...
			<div class="bbs_list">
				<table id="bbs_list1">
						...
						<tr>
						  ...
						  <th scope="col">대면평가일자</th>
						  <th scope="col">위원회명</th>
						  <th scope="col">평가구분</th>
						  <th scope="col">평가완료여부</th>
						  <th scope="col">간사(이름)</th>
						  <th scope="col">평가장소</th>
						  <th scope="col">평가점수</th>
						  <th scope="col">대면평가결과명</th>
						  <th scope="col">대면이의신청일자</th>
						  <th scope="col">대면평가이의신청결과명</th>							  
						</tr>
						</thead>
...
;(function() {
	/* $(document).on('click', '.pageClass', function() {
		...
		$.ajax({
			url : '/empo/schedule/selectProjectList2.do'
			, type : 'post'
			, data :formSer
		}).done(function(res) { 
			console.log(res);
	...
	var projectListView = function(row) {
		...
		var data = 
			...
			'	<td class="subRow">' + row.totDvlpSrtYmd  + '</td>' +
			'	<td class="subRow">' + row.totDvlpEndYmd + '</td>' +
	...
	$(function() { 
		var formSer = $("#searchForm1").serialize();
		//과제리스트 호출
		$.post('/empo/schedule/selectProjectList2.do', formSer).done(function(res) {
	...
	$(document).on('click', '.pageClass', function() {
		...
		$.ajax({
			url : '/empo/schedule/seProjectList.do'
			, type : 'post'
			, data :formSer
		}).done(function(res) { 
			console.log(res);
	...
	var projectListView = function(row) {
		...
		var data = 
			...
			'	<td class="subRow">' + row.ymd2 + '</td>' +
			'	<td class="subRow">' + row.rsltNm2 + '</td>' +
			'</tr>' ;
	...
	$(function() {
		...
		$.post('/empo/schedule/seProjectList.do', formSer).done(function(res) {
			console.log(res.list);
			res.list.forEach( function(row, index) {
				$('#resultList1').append(projectListView(row));
			});
