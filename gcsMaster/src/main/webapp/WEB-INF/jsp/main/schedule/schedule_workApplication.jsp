			...
			<div class="bbs_list">
				<table id="bbs_list1">
						...
						<tr>
						  ...
						  <th scope="col">접수마감일자</th>
						  <th scope="col">서류제출완료일자</th>
						</tr>
						</thead>
...
;(function() {
	...
	$(document).on('click', '.pageClass', function() {
		...
		$.ajax({
			url : '/empo/schedule/ApplicationProjectList.do'
			, type : 'post'
			, data :formSer
		}).done(function(res) { 
			console.log(res);
	...
	var projectListView = function(row) {
		...
		var data = 
			...
			'	<td class="subRow"></td>' +
			'	<td class="subRow">' + row.ancmYmd  + '</td>' +
			'	<td class="subRow">' + row.ancmNm + '</td>' +
			'	<td class="subRow">' + row.dtlAncmNm  + '</td>' +
			'	<td class="subRow">' + row.rcptClosYmd + '</td>' +
			'	<td class="subRow">' + row.submCmptYmd + '</td>' +
			'</tr>' ;
	...
	$(function() {
		...
		$.post('/empo/schedule/ApplicationProjectList.do', formSer).done(function(res) {
